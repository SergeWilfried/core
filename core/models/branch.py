...
class BranchSettings(BaseModel):
    ...
    allow_account_opening: bool = Field(
        default=True, description="Allow new account opening"
    )
    allow_loan_processing: bool = Field(
        default=False, description="Allow loan processing"
    )

    # Compliance settings (inherits from org, can be stricter)
    require_manager_approval_above: Optional[float] = Field(
        None, description="Amount requiring branch manager approval"
    )
    compliance_level_override: Optional[str] = Field(
    ...
    operating_hours: Optional[dict] = Field(
        None,
        description="Operating hours by day: {'monday': {'open': '09:00', 'close': '17:00'}}"
    )
...
class Branch(BaseModel):
    ...
    phone: Optional[str] = Field(None, description="Branch phone")
    ...
    # Branch manager
    manager_user_id: Optional[str] = Field(None, description="Branch manager user ID")

    # Status
    status: BranchStatus = Field(
        default=BranchStatus.PENDING, description="Branch status"
    )
    ...
    opened_at: Optional[datetime] = Field(
        None, description="When branch opened for business"
    )
    closed_at: Optional[datetime] = Field(
        None, description="When branch closed"
    )
    ...
    def get_effective_transaction_limit(
        self, org_limit: Optional[float]
    ) -> Optional[float]:
        """
    ...
    def get_effective_compliance_level(
        self, org_compliance_level: str
    ) -> str:
        """
        ...
        if self.settings.compliance_level_override:
            ...
            branch_level = strictness.get(
                self.settings.compliance_level_override, 2
            )
...
class BranchAssignment(BaseModel):
    ...
    user_id: Optional[str] = Field(None, description="User ID if assigning user")
    resource_type: Optional[str] = Field(
        None, description="Resource type: account, customer, etc."
    )
    resource_id: Optional[str] = Field(None, description="Resource ID")

    # Assignment details
    role_at_branch: Optional[str] = Field(
        None, description="User's role at this branch (if user assignment)"
    )
    is_primary: bool = Field(
        default=True, description="Primary assignment for this user/resource"
    )
...
class BranchPerformanceMetrics(BaseModel):
    ...
    total_transaction_volume: float = Field(
        default=0.0, description="Total transaction volume"
    )
    average_transaction_amount: float = Field(
        default=0.0, description="Average transaction amount"
    )
    ...
    # Compliance metrics
    compliance_checks_performed: int = Field(
        default=0, description="Compliance checks"
    )
    transactions_blocked: int = Field(
        default=0, description="Transactions blocked"
    )
    manual_reviews_required: int = Field(
        default=0, description="Manual reviews required"
    )
    average_risk_score: float = Field(
        default=0.0, description="Average risk score"
    )
